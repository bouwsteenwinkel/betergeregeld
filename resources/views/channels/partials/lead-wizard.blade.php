@php
    ...
    $goals = [
        ['value' => 'meer_klanten',  'label' => 'Meer klanten & afspraken',   'icon' => '📈', 'facet' => 'website'],
        ['value' => 'professioneel', 'label' => 'Professioneler overkomen',    'icon' => '✨', 'facet' => 'website'],
        ['value' => 'vindbaar',      'label' => 'Beter vindbaar in Google',    'icon' => '🔍', 'facet' => 'website'],
        ['value' => 'webshop',       'label' => 'Online verkopen (webshop)',   'icon' => '🛍️', 'facet' => 'webshop'],
    ];
...
<section id="gratis-voorbeeld" class="lwz-band">
    <div class="lwz-wrap">
        <div class="lwz-head">
        ...
        <form class="lwz" method="POST" action="{{ $site->url('contact') }}" novalidate>
            ...
            <div class="lwz-step is-active" data-step="1">
                <p class="lwz-q">Hoe heet je zaak?</p>
                <input class="lwz-input" type="text" name="company" value="{{ old('company') }}"
                       placeholder="bijv. {{ $zaakNaam }}" data-required autofocus>
                <div class="lwz-nav"><span></span><button type="button" class="lwz-next">Volgende →</button></div>
            </div>

            {{-- 2 ─ Huidige website --}}
            <div class="lwz-step" data-step="2">
                <p class="lwz-q">Heb je nu al een website?</p>
                <p class="lwz-help">Plak 'm hier zodat we eraan kunnen meten — of typ gewoon "nee".</p>
                <input class="lwz-input" type="text" name="current_website" value="{{ old('current_website') }}"
                       placeholder="www.jouwzaak.nl — of: nee">
                <div class="lwz-nav"><button type="button" class="lwz-back">← Terug</button><button type="button" class="lwz-next">Volgende →</button></div>
            </div>

            {{-- 3 ─ Doel (bepaalt de groeifase) --}}
                ...
                <div class="lwz-opts" data-choice="goal" data-required data-advance>
                    @foreach ($goals as $g)
                        <button type="button" class="lwz-opt" data-value="{{ $g['value'] }}" data-facet="{{ $g['facet'] }}">
                            <span class="lwz-opt-ic">{{ $g['icon'] }}</span>{{ $g['label'] }}
                        </button>
                    @endforeach
                </div>
            ...
            {{-- 5 ─ Gewenste functies (meerkeuze) --}}
            <div class="lwz-step" data-step="5">
                <p class="lwz-q">Wat moet er zeker op?</p>
                <p class="lwz-help">Kies wat voor jou belangrijk is — meerdere mag.</p>
                @if ($featureOpts)
                    <div class="lwz-checks">
                        @foreach ($featureOpts as $key => $label)
                            <label class="lwz-check">
                                <input type="checkbox" name="features[]" value="{{ $key }}"
                                    @checked(collect((array) old('features'))->contains($key))>
                                <span>{{ $label }}</span>
                            </label>
                        @endforeach
                    </div>
                @else
                    <p class="lwz-help">We bespreken de gewenste functies tijdens je afspraak.</p>
                @endif
                <div class="lwz-nav"><button type="button" class="lwz-back">← Terug</button><button type="button" class="lwz-next">Volgende →</button></div>
            </div>

            {{-- 6 ─ Timing --}}
            <div class="lwz-step" data-step="6">
                <p class="lwz-q">Wanneer wil je live?</p>
                <div class="lwz-opts" data-choice="timing" data-required data-advance>
                    @foreach ($timings as $t)
                        <button type="button" class="lwz-opt" data-value="{{ $t['value'] }}">{{ $t['label'] }}</button>
                    @endforeach
                </div>
                <div class="lwz-nav"><button type="button" class="lwz-back">← Terug</button><span></span></div>
            </div>
            ...
            {{-- 7 ─ Contactgegevens --}}
            <div class="lwz-step" data-step="7">
                <p class="lwz-q">Waar mogen we je voorbeeld naartoe sturen?</p>
                <input class="lwz-input" type="text"  name="contact_name" value="{{ old('contact_name') }}" placeholder="Je naam" data-required>
                <input class="lwz-input" type="email" name="email"        value="{{ old('email') }}"        placeholder="Je e-mailadres" data-required>
                <input class="lwz-input" type="tel"   name="phone"        value="{{ old('phone') }}"        placeholder="Je telefoonnummer" data-required>
                <div class="lwz-nav"><button type="button" class="lwz-back">← Terug</button><button type="button" class="lwz-next">Volgende →</button></div>
            </div>

            {{-- 8 ─ Afspraak --}}
            <div class="lwz-step" data-step="8">
                <p class="lwz-q">Hoe bespreken we je voorbeeld?</p>
                <div class="lwz-opts" data-choice="appointment_type" data-required>
                    <button type="button" class="lwz-opt" data-value="onsite"><span class="lwz-opt-ic">☕</span>Liever langskomen <small>(omgeving Bussum)</small></button>
                    <button type="button" class="lwz-opt" data-value="meet"><span class="lwz-opt-ic">💻</span>Online via Google&nbsp;Meet</button>
                </div>
...
<style>
    ...
    .lwz-bar{display:block;height:100%;width:12.5%;background:var(--lwz-accent);border-radius:999px;transition:width .35s ease}
    ...
    .lwz-opts{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:12px;margin-top:18px}
    ...
    .lwz-opt-ic{font-size:20px;line-height:1}
    ...
    .lwz-next,.lwz-submit{font:inherit;font-size:15px;font-weight:600;cursor:pointer;color:#fff;
        background:var(--lwz-accent);border:1px solid var(--lwz-accent);border-radius:999px;padding:13px 26px;transition:.16s}
    .lwz-next:hover,.lwz-submit:hover{filter:brightness(.94)}
    .lwz-submit{width:100%}

    .lwz-errors{max-width:680px;margin:0 auto 18px;padding:14px 18px;border-radius:var(--radius,14px);
