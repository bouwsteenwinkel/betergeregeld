@php $start = \Carbon\Carbon::parse($appt->starts_at)->setTimezone($tz); @endphp
<!DOCTYPE html>
<html lang="nl">
...
<body style="margin:0;background:#f4f7fa;font-family:Arial,Helvetica,sans-serif;color:#1c2530">
    <div style="max-width:560px;margin:0 auto;padding:28px 20px">
        <div style="background:#fff;border-radius:14px;padding:28px 26px;box-shadow:0 10px 30px -18px rgba(0,0,0,.3)">
            <h1 style="font-size:20px;margin:0 0 6px">Je afspraak is bevestigd</h1>
            <p style="margin:0 0 18px;color:#5b6b78">Hoi {{ $appt->name }}, we hebben je afspraak ingepland. Tot dan!</p>
            ...
            @else
                <p style="margin:20px 0 0;color:#5b6b78">De Google Meet-link ontvang je nog. De afspraak staat in je agenda via het bijgevoegde bestand (afspraak.ics).</p>
            @endif

            <p style="margin:22px 0 0;font-size:13px;color:#5b6b78">Kan het niet doorgaan? Beantwoord deze mail, dan verzetten we het.</p>
        </div>
